...
public class CleaningFHVMapper extends Mapper<Void, Group, Text, Text> {
    ...
    public void map(Void key, Group record, Context context) 
        ...
        try {
            ...
            if (!ValidationUtils.isValidNumber(tripMiles, 0.0, 100.0)) {
                valid = false;
            }
            ...
            if (valid) {
                ...
                cleanedRecord.put("pickup_datetime", pickupDt);
                cleanedRecord.put("dropoff_datetime", dropoffDt);
                cleanedRecord.put("PULocationID", Integer.parseInt(puLocation));
                cleanedRecord.put("DOLocationID", Integer.parseInt(doLocation));
                cleanedRecord.put("trip_miles", Double.parseDouble(tripMiles));
                cleanedRecord.put("taxi_type", "fhv");
